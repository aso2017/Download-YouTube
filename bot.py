...
# Render filesystem is read-only outside writable temporary storage
...
def download_youtube(url: str, workdir: Path) -> tuple[Path, str]:
    ...
    # Only pass cookiefile when the Render Secret File actually exists
...
async def handle_url(
    ...
    # Writable temporary directory on Render
    ...
    try:
        ...
        # Remove downloaded files after sending/error
        shutil.rmtree(
            workdir,
            ignore_errors=True
        )
...
async def main():
    ...
    print(
        f"YouTube cookies: {cookies_status}"
    )
